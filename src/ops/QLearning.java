...
public class QLearning {
    ...
    private final int statesCount = boardHeight * boardWidth;

    private final int reward = 100;
    private final int penalty = -10;

    private char[][] board;  // Maze read from file
    private int[][] R;       // Reward lookup
    private double[][] Q;    // Q learning

    public void init() {
        ConNN.init();
        File file = new File(Config.qLearnCache);
        ...
        try (FileInputStream fis = new FileInputStream(file)) {
            ...
            for (int k = 0; k < statesCount; k++) {
                ...
                // We will navigate with i and j through the board, so we need
                ...
                // If not in final state or a wall try moving in all directions in the board
                if (board[i][j] != 'F') {

                    // Try to move left in the board
                    int goLeft = j - 1;
                    if (goLeft >= 0) {
                        int target = i * boardWidth + goLeft;
                        if (board[i][goLeft] == '0') {
                            R[k][target] = 0;
                        } else if (board[i][goLeft] == 'F') {
                            R[k][target] = reward;
                        } else {
                            R[k][target] = penalty;
                        }
                    ...
                    // Try to move right in the board
                    int goRight = j + 1;
                    if (goRight < boardWidth) {
                        int target = i * boardWidth + goRight;
                        if (board[i][goRight] == '0') {
                            R[k][target] = 0;
                        } else if (board[i][goRight] == 'F') {
                            R[k][target] = reward;
                        } else {
                            R[k][target] = penalty;
                        }
                    ...
                    // Try to move up in the board
                    int goUp = i - 1;
                    if (goUp >= 0) {
                        int target = goUp * boardWidth + j;
                        if (board[goUp][j] == '0') {
                            R[k][target] = 0;
                        } else if (board[goUp][j] == 'F') {
                            R[k][target] = reward;
                        } else {
                            R[k][target] = penalty;
                        }
                    ...
                    // Try to move down in the board
                    int goDown = i + 1;
                    if (goDown < boardHeight) {
                        int target = goDown * boardWidth + j;
                        if (board[goDown][j] == '0') {
                            R[k][target] = 0;
                        } else if (board[goDown][j] == 'F') {
                            R[k][target] = reward;
                        } else {
                            R[k][target] = penalty;
                        }
    ...
    boolean isFinalState(int state) {
        int i = state / boardWidth;
        int j = state - i * boardWidth;
